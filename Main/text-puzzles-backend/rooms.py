...
rooms_dict['mage_tower_room_entrance_lvl_1'] = {
  "room": {
    ...
    "room_description": "You find yourself standing in a clearing, surrounded by the gentle rustle of leaves and the soft creaking of trees in the distance. The air is filled with an eerie silence, punctuated only by the ominous voice that seems to come from all around you. Before you lies a beautiful, intricate magic circle, etched into the floor with swirling runes and a glowing silver outline. The circle seems to pulse with a soft, purple light, casting an otherworldly glow across the clearing. You also notice that you're holding a note on a piece of paper, saying:\n\n Welcome to my tower! MWahauhauhuahu. Let's see if you can reach the top. To pass the first level, you must speak the pass word. Hint: It's \"Orange\". You must whisper the pass word for it to be considered. Be careful! Any whisper will count as an attempt to guess the password."
  }
}
  ...
  "room": {
    "description": {
        "prompts": {
            ...
            "loss_condition": "The player loses when the counter of their magic use reaches zero (0), as determined by the magical_usage_counter game object. Do not count the player's attempts yourself, rather rely on the magical_usage_counter state.",
            "player_entrapment": "The player is initially free to move as they wish, except that, if they leave the clearing they're in and wander through the forest, they'll return to the clearing after walking around in circles. While the mysterious_fog state is active, the player is always teleported to inside the magic circle and is unable to leave it. That does not prevent them from using magic to affect objects outside the circle.",
            ...
            "mystery_book": "This object is initially inactive. This object represents the book that appears with the mystery_table. It contains a secret message within it's pages, which says 'Tip: whisper \"Placate\", but don't let them see... MWahuahauha!!'. However, the player initially can't see this message, given that only the first page of the book is initially displayed, and that page is blank. The player can see the other pages if they manage to turn the book pages somehow. Other than those characteristics, this is a normal book.",
            "mystery_book_status": "This object is initially inactive. This tracks whether the visible page of the mystery_book is just a blank page (initial state) or if that state has changed to another page (for example, if the player interacts with the book containing this object). The page is initially blank, concealing the rest of the book's contents. Merely activating the object is not enough for the page to turn. If and only if the page turns, reveal the secret message within.",
            ...
            "mysterious_fog":"This object is initially inactive. It should only activate when the whisper_guardian object has its condition fulfilled. All game objects are affected by it at the same time. While active, the fog obscures the environment, including the sky, but it does not obscure objects protected by the mystery_candle. The fog is very light and can be dispersed by a moderately strong wind, in which case this object becomes inactive and the whisper_guardian is activated again. This object should always state to the game engine: 'Whenever this object is activated or deactivated, notify all other game objects'.",
            "general_reminder_1":"This object reminds the game engine of general rules. Always state these rules to the game engine. Rule 1-1: Do not talk about game objects directly to the player, especially inactive or abstract game objects. You, as a rule reminder, should not keep track of other rules not specified here.",
        },
        "responsibilities": {
            ...
            "mystery_book_status": "This object tracks the state of a certain page in a specific mistery_book and its displayed status",
            ...
            "general_reminder_1":"This object reminds the game engine of general rules. It should always be queried when determining player success, with a question similar to 'what general rules should I remember?'",
        }
    },
    "winning_message": "You observe a tall stone tower materialize in front of you! It looks old, sturdy and imponent. You can now enter it. The password to for the next room is \"skybox\"  \n\n\n",
    "losing_message": "You were caught using magic too much! You lose.\n",
    "room_description": "You find yourself in a forest clearing with a light purple magic circle on the ground, surrounded by trees. The air is crisp and quiet, with no signs of disturbance. You notice that you are holding a magical note and a blank spellbook, but there's nothing out of the ordinary about them at the moment. The magical note in your hand displays a message: \"Have you already whispered the secret word? It was revealed at the end of the previous level\""
  }
}